...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        //Given
        Board project = prepareTestData();
        long resultExpected = 10L;
        //When
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        long daysFormBegining = (project.getTaskLists().stream())
                .filter(inProgressTasks::contains)
                .flatMap(t1 -> t1.getTasks().stream())
                .map(t -> t.getCreated())
                .map(d -> DAYS.between(d, LocalDate.now()))
                .reduce(Long::sum).orElse(0L);
        int howManyTasks = (project.getTaskLists().stream())
                .filter(inProgressTasks::contains)
                .flatMap(t1 -> t1.getTasks().stream())
                .map(t -> 1)
    ...
    @Test
    public void testAddTaskListAverageWorkingOnTaskPart2() {
        //Given
        Board project = prepareTestData();
